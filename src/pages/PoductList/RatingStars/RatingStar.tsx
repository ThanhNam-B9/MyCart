...
export default function RatingStar({ queryConfig }: Props) {
  ...
  const handleFilterStar = (star: number) => {
    navigate({
      pathname: path.home,
      search: createSearchParams(omit({ ...queryConfig, rating_filter: '' + star, page: '1' }, ['order'])).toString()
    })
  }
  return (
    <div>
                ...
                >
                  {Array(5)
                    ...
                    .map((_, index) => {
                      ...
                      return (
                        <svg viewBox='0 0 30 30' className='w-5 h-3 ' key={index}>
                          <defs>
